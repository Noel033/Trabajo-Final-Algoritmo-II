...
# GIFs para cada illa robada (usando solo las illas disponibles)
ILLAS_GIFS = {
    "Tumi": "tumi.gif",   
    "Chacana": "chacana.gif", 
    "Illa": "illa.gif",
...
class MochilaVisual:
    ...
    def __init__(self):
        ...
        self.y = 10  # Parte superior
        self.illas_guardadas = []
        self.font = pygame.font.SysFont("Arial", 12, bold=True)
        
        # Cargar GIF de la mochila si existe
        self.mochila_gif = None
        mochila_path = "mochila.gif"  # Puedes cambiar por tu GIF de mochila
    ...
    def draw(self, screen):
        """Dibujar la mochila y las illas guardadas"""
        if self.mochila_gif:
            # Solo dibujar el GIF de la mochila centrado
            screen.blit(self.mochila_frames[self.current_frame], (self.x, self.y))
        else:
            # Si no hay GIF, dibujar mochila simple
            ...
            # Dibujar correas de la mochila
            pygame.draw.rect(screen, (64, 64, 64), (self.x + 10, self.y - 5, 20, 8))
            pygame.draw.rect(screen, (64, 64, 64), (self.x + self.width - 30, self.y - 5, 20, 8))
        
        # Título de la mochila
        title_text = self.font.render("MOCHILA DE EKEKO", True, WHITE)
        title_rect = title_text.get_rect(center=(self.x + self.width // 2, self.y + 15))
        screen.blit(title_text, title_rect)
        
        # Mostrar illas guardadas
        if self.illas_guardadas:
            illas_text = f"Illas: {len(self.illas_guardadas)}/19"
            count_text = self.font.render(illas_text, True, YELLOW)
            count_rect = count_text.get_rect(center=(self.x + self.width // 2, self.y + 35))
            screen.blit(count_text, count_rect)
            
            # Mostrar algunas illas específicas (máximo 3)
            display_illas = self.illas_guardadas[:3]
            for i, illa in enumerate(display_illas):
                illa_text = self.font.render(f"• {illa}", True, WHITE)
                illa_rect = illa_text.get_rect(x=self.x + 10, y=self.y + 50 + i * 12)
                screen.blit(illa_text, illa_rect)
            
            if len(self.illas_guardadas) > 3:
                more_text = self.font.render(f"... y {len(self.illas_guardadas) - 3} más", True, GRAY)
                more_rect = more_text.get_rect(x=self.x + 10, y=self.y + 50 + 3 * 12)
                screen.blit(more_text, more_rect)
        else:
            empty_text = self.font.render("Vacía", True, GRAY)
            empty_rect = empty_text.get_rect(center=(self.x + self.width // 2, self.y + 50))
